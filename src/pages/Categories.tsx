...
const Categories = () => {
  ...
  const handleDelete = (categoryId: number) => {
    if (categoryId) {
      ...
      const updatedTasks = user.tasks.map((task) => {
        const updatedCategoryList = task.genres?.filter((category) => category.id !== categoryId);
        return {
          ...task,
      ...
      toast.success(() => (
        <div>
          Deleted genre - <b>{categoryName}.</b>
        </div>
      ));
    }
  };
  ...
  const handleAddCategory = () => {
    if (name !== "") {
      ...
      toast.success(() => (
        <div>
          Added Genre - <b>{newCategory.name}</b>
        </div>
      ));
      setUser((prevUser) => ({
        ...prevUser,
      ...
      toast.error((t) => <div onClick={() => toast.dismiss(t.id)}>Genre name is required.</div>);
    }
  };
  ...
  const handleEditCategory = () => {
    if (selectedCategoryId) {
      ...
      const updatedTasks = user.tasks.map((task) => {
        const updatedCategoryList = task.genres?.map((category) => {
          if (category.id === selectedCategoryId) {
            return {
  ...
  if (!user.settings[0].enableCategories) {
    return <NotFound message="Genre are not enabled." />;
  }
    ...
    <>
      ...
      <Container>
        ...
        >
          ...
          <DialogContent>
            This will remove the genre from your list and associated tasks.
          </DialogContent>
