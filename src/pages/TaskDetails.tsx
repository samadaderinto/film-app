...
const TaskDetails = () => {
  ...
  if (!task) {
    return (
      <NotFound
        message={
          <div>
            Film with id <PathName>{formattedId}</PathName> was not found.
          </div>
        }
      />
    );
  ...
  useEffect(() => {
    document.title = `Filmo - ${task.name}`;
  }, []);

  return (
    <>
      <TopBar title="Film Details" />
      <Container>
        <TaskName>Film: {task.name}</TaskName>
        <TaskTable>
          <tbody>
            <TableRow>
              ...
              <TableData>
                <CategoryContainer>
                  {task?.genres?.map((genre) => (
                    <CategoryBadge key={genre.id} category={genre} glow={false} />
                  ))}
                </CategoryContainer>
              </TableData>
            </TableRow>
