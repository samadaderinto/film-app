...
import { getFontColor, timeAgo } from "../utils";

const UserSettings = () => {
  const { user, setUser } = useContext(UserContext);
  const { email, profilePicture, createdAt } = user;
  ...
  useEffect(() => {
    document.title = `Filmo App - User ${email ? `(${email})` : ""}`;
  }, [email]);
    ...
    <>
      ...
      <Container>
          ...
          >
            ...
            >
              {!profilePicture && userName
                ...
                : !user.profilePicture && !userName && email
                ? email[0].toUpperCase()
                : undefined}
            </Avatar>
          </Badge>
        </Tooltip>
        <UserName translate="no">{email || "User"}</UserName>
        <Tooltip
        ...
        <SaveBtn
          ...
          disabled={userName.length > USER_NAME_MAX_LENGTH || userName === email}
        >
          Save name
        </SaveBtn>
        <Button
      ...
      <Dialog open={logoutConfirmationOpen} onClose={handleLogoutConfirmationClose}>
        ...
        <DialogContent>
          Are you sure you want to logout? <b>Your films will not be saved.</b>
        </DialogContent>
        <DialogActions>
