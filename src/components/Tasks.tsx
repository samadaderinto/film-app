...
import { formatDate, getFontColor } from "../utils";
  ...
  const reorderTasks = (tasks: Task[]): Task[] => {
    ...
    if (selectedCatId !== undefined) {
      unpinnedTasks = unpinnedTasks.filter((task) => {
        if (task.genres) {
          return task.genres.some((category) => category.id === selectedCatId);
        }
        return false;
      });
  ...
  useEffect(() => {
    ...
    uniqueCategories.forEach((category) => {
      const categoryTasks = tasks.filter((task) =>
        task.genres?.some((cat) => cat.id === category.id)
      );
  ...
  return (
    <>
      ...
      <TasksContainer>
        {user.tasks.length > 0 && (
          <SearchInput
            focused
            color="primary"
            placeholder="Search for film by title, description..."
            autoComplete="off"
            value={search}
            onChange={(e) => {
        ...
        {search && reorderTasks(user.tasks).length > 1 && user.tasks.length > 0 && (
          ...
          >
            <b>
              Found {reorderTasks(user.tasks).length} film
        ...
        {user.tasks.length !== 0 ? (
          reorderTasks(user.tasks).map((task) => (
            ...
            >
              ...
              <TaskInfo translate="no">
                ...
                <TaskHeader>
                  <TaskName>{highlightMatchingText(task.name, search)}</TaskName>
                  <Tooltip
          ...
          <NoTasks>
            <b>You don't have any film yet</b>
            <br />
            ...
            const updatedTasks = user.tasks.map((task) => {
              if (task.id === editedTask.id) {
                return {
                  ...
                  category: editedTask.genres || undefined,
                  lastSave: new Date(),
                };
              }
              return task;
      ...
      <Dialog open={deleteDialogOpen} onClose={cancelDeleteTask}>
        <DialogTitle>Are you sure you want to delete the film?</DialogTitle>
        <DialogContent>
          {selectedTask.emoji !== undefined && (
            <>
              ...
              <p>
                <b>film Name:</b> {selectedTask.name}
              </p>
              {selectedTask.description && (
                <p>
                  <b>film Description:</b> {selectedTask.description}
                </p>
              )}
              {selectedTask.genres?.[0]?.name && (
                <p>
                  <b>{selectedTask.genres.length > 1 ? "Categories" : "Category"}:</b>{" "}
                  {listFormat.format(selectedTask.genres.map((cat) => cat.name))}
                </p>
              )}
            </>
          )}
